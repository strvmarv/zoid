...
- `default_model().to_string()` → `default_model(&app.registry)` at lines 2527 and 4343 (the new free fn returns `String`, so drop the trailing `.to_string()`).
